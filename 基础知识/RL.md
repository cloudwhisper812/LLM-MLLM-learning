...
5. “好回答的概率反而下降了” (Likelihood Drop)： 这是 DPO 面试必考题。在观察 Loss 曲线时，你经常会发现，随着训练进行，模型对 $y_w$ 的预测概率其实在下降.因为 DPO 的目标是扩大 $(y_w - y_l)$ 的差值。有时候模型发现，把 $y_l$ 的概率疯狂往下压，比把 $y_w$ 的概率往上抬要容易得多。结果就是两者的概率都在降，但 $y_l$ 降得更猛，差值还是变大了，Loss 依然在降。后果：这会导致模型生成质量退化（因为整体生成概率被压低了）。解决：后续有诸多魔改版本（比如 cDPO 引入标签平滑，或直接在 Loss 里加一个对 $y_w$ 的负对数似然 NLL 惩罚项），强制模型不要拉低 $y_w$ 的概率。
6. 长度作弊 (Length Hack)： 如果你的数据里，好回答普遍比差回答长，DPO 很容易学到一个捷径：“只要字数多就是好回答”。这会导致模型变得非常啰嗦。因此在构建三元组数据时，控制 $y_w$ 和 $y_l$ 的长度尽量一致是基本功
