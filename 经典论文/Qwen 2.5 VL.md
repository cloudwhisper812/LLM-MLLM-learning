...
## Post training
#### Rejection Sampling
- 做法：模型自己做题，做对的把它的cot保留当作训练数据。打错的直接丢弃。数据清洗的话，用gt答案，格式清洗（去掉废话，复读机等脏数据），并且确保模型不是瞎猜而是真的用了多模态数据。
- 迭代飞轮： 中间模型生成数据 -> 过滤数据 -> 训练更好模型 -> 生成数据 -> ...

#### SFT + DPO
- 两个阶段都冻结ViT。ViT在预训练阶段已经有很强的特征提取能力，这里freeze可以节省显存和计算。
- SFT: 多样化数据上训练。
- DPO: 聚焦图文和纯文本数据，把能力泛化到视频上。SFT是让模型学会解题，告诉模型什么是对的。DPO通过好坏回答的对比，让模型在回答风格，逻辑连贯性上等更符合人类喜好。
- DPO数据收集：同一个问题模型生成多个回答（1）根据数学代码答案来判断对错。（2）更强模型来判断连贯性，幻觉等 （3）之前的reject sampling数据 （4）人工标注
